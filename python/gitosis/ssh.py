...
def filterAuthorizedKeys(fp):
    ...
    for line in fp:
        ...
        if _COMMAND_RE.match(line):
            continue
        yield line
#writeAuthorizedKeys首先把原有目录下即(~/.ssh/authorized_keys)文件
#的内容复制到现在的tmp临时文件中，然后再把
#repositories/gitosis-admin.git/gitosis-export/keydir目录下的pub
#文件中的内容写入到tmp文件中，最后再重命名此文件为authorized_keys
